...
import { POST } from '@/app/api/ai/copy/route'
...
const validBody = {
  product: 'HypeFlow CRM',
  audience: 'leads em qualificação',
  objective: 'fechar' as const,
...
function makeRequest(body: unknown, contentLength?: number) {
  ...
  return new NextRequest('http://localhost/api/ai/copy', {
    method: 'POST',
    headers: {
...
describe('POST /api/ai/copy', () => {
  ...
  it('returns 413 when body exceeds 16KB', async () => {
    const res = await POST(makeRequest(validBody, 20_000))
  ...
  it('returns 400 when body is invalid (missing required fields)', async () => {
    ...
    const res = await POST(makeRequest({ product: 'test' }))
