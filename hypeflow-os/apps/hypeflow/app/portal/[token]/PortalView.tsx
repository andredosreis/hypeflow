...
/* Mock portal data (replaced by real DB data in a future product story).
 * Deterministic on validated clientId so the same client always sees the
 * same demo dataset within a session. */
function getMockPortalData(clientId: string): PortalData {
  const seed = (clientId.charCodeAt(0) + clientId.charCodeAt(clientId.length - 1)) % 6
...
/* ─── Main portal view ─── */
export default function PortalView({ clientId }: { clientId: string; agencyId: string }) {
  ...
  useEffect(() => {
    const d = getMockPortalData(clientId)
    setData(d)
